...
def fetchNounRelations(request):
    ...
    if len(nounRelationsList) == 0:
        ...
        response.status_code = 202  # To announce that the user isn't allowed to publish        return response
...
def feedBack(request):
    if request.method == "GET":
        ...
        comment = str(request.POST.get("comments", None))
        print(name)
        print("post")
        print(email)
        print(comment)
        return render(request, "index.html", {"found": True, "suggFound": False, "feedBack": True})
